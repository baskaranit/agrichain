...
function Edit({ history, match }) {
    ...
    useEffect(() => {
        ...
        productService.getById(id).then(user => {
            const fields = ['name', 'price', 'discount_count', 'discount_price'];
            fields.forEach(field => {
                let value = user[field];
                if(field == "discount_price")
                    value = user["discount"].price;
                else if(field == "discount_count")
                    value = user["discount"].count;
